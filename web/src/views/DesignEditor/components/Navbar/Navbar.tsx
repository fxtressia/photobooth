...
import { loadGraphicTemplate, parseJSON, useRemoteDesignSaver } from "~/utils/load-and-save"
...
const Navbar = () => {
  ...
  */
  ...
  const makeDownloadTemplate = async () => {
    ...
    const json = parseJSON(editor, currentDesign, scenes);
    if (json) {
      return makeDownload(json);
    }
    /*if (editor) {
  ...
  */
  const handleImportTemplate = React.useCallback(
    async (data: any) => {
      let template = await loadGraphicTemplate(data, editor);
      //   @ts-ignore
  ...
  return (
    ...
    <ThemeProvider style={{ position: "sticky", top: 0, left: 0 }} theme={DarkTheme}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", height: "1.5rem", paddingLeft: "15px", backgroundColor: "#6e6eff" }}>
        ...
        <div style={{ display: "flex", color: "#ffffff", gap: "25px", alignItems: "center" }}>
